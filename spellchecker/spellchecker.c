...
int main(int argc, char * argv[]) {
    ...
    while (fgets(word, MAX_WORD_LEN, stdin)) { //grab line from standard input and store it in word
        ...
        for (i = 0; i < lineCount; i++) {
            ...
            destroyNewLineCharacter(workingWord);
            int workingWordlength = strlen(workingWord) - 1;
            // Get length of dictionary word
            toLowerCase(workingWord); //covert dictionary word to lowercase
            if (workingWordlength == wordlength) { // if length equals workingWordLen
                ...
                if (workingWordlength > wordlength) { // figures out which word is longer and assigns length
                ...
                if (workingWordlength > wordlength) { // Creates smaller and bigger strings, so we know which is which
                ...
                for (j = 0; j < bigStrlength; j++) { // index the big string
                    int pos = 0;
        ...
        if (match != -1) { // if match was found, print out match
...
void loadWordList(char * fileName, int * count) { // this function loads words from file into array and counts words as it goes
    int i = 0;
...
int hammingDistance(char * a, char * b, int length) { // this function calculates hammingDistances
